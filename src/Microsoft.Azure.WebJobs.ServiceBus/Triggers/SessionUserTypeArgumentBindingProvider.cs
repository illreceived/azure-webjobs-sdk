...
{
    internal class SessionUserTypeArgumentBindingProvider : ISessionQueueTriggerArgumentBindingProvider
    {
        ...
        private static ITriggerDataArgumentBinding<BrokeredMessageSession> CreateBinding(Type itemType)
        {
            Type genericType = typeof(UserTypeArgumentBinding<>).MakeGenericType(itemType);
            return (ITriggerDataArgumentBinding<BrokeredMessageSession>)Activator.CreateInstance(genericType);
        }

        private class UserTypeArgumentBinding<TInput> : ITriggerDataArgumentBinding<BrokeredMessageSession>
        {
            ...
            public async Task<ITriggerData> BindAsync(BrokeredMessageSession value, ValueBindingContext context)
            {
                ...
                TInput contents = await GetBody(value.Message, context);
